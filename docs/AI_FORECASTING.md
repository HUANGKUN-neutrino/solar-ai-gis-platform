...
LightGBM / XGBoost 用于短期功率预测和表格特征建模，适合快速处理天气、装机容量和时间特征。Transformer 作为长序列预测扩展方案，用于在数据量更充分时建模历史发电曲线、天气变化和未来趋势。

```mermaid
flowchart LR
  Input["装机容量 / 天气 / 云量 / 降雨 / 时间特征"] --> Feature["特征工程"]
  Feature --> Normalize["特征归一化与样本构造"]
  Normalize --> ML["LightGBM / XGBoost"]
  Normalize --> Transformer["Transformer Extension"]
  ML --> Calibrate["业务校准与容量缩放"]
...
| 峰值功率 | 估算日内峰值发电能力 |
| 趋势判断 | 判断未来多日发电趋势 |
| 降级标识 | 标识结果来自 LightGBM / XGBoost、Transformer 扩展或公式模型 |

## 模型组合策略

平台采用“规则测算 + LightGBM / XGBoost + Transformer 扩展”的组合方式，而不是只依赖单一模型。
...
```mermaid
flowchart TB
  Request["预测请求"] --> M{"LightGBM / XGBoost 可用?"}
  M -- 是 --> R1["短期功率预测"]
  M -- 否 --> T{"Transformer 扩展可用?"}
  T -- 是 --> R2["长序列预测"]
  T -- 否 --> R3["公式模型估算"]
  R1 --> Output["统一输出格式"]
  R2 --> Output
